...
namespace EasyItemCreator
{
    public partial class Form1 : Form
    {
        ...
        {
            int wynik = 0;
            if (addon_check.Checked == true)
            {
                wynik = 1;
            }
            return wynik;
        }
        private void CreateList(int start, string text)
        {
            // VNUM	LOCALE_NAME
            int vnum = start;
            string name = text + " +" + vnum % 10;
            richTextBox2.AppendText(vnum + "	" + name + Environment.NewLine);
        }
        private void CreateList_client(int start, string type, string icon_location, string ground_mesh)
        {
            // VNUM	LOCALE_NAME
            int vnum = start;
            string unique = icon_location;
            if (!String.IsNullOrEmpty(ground_mesh))
            {
                unique += "	";
                unique += "meshes/item/";
                unique += ground_mesh;
            ...
            {
                unique += "	";
                unique += "meshes/item/item_bag.gr2";
            }
            client_item_list.AppendText(vnum + "	" + type + "	" + unique + Environment.NewLine);
        }
        private void button1_Click(object sender, EventArgs e)
        {
            richTextBox1.Clear();
            richTextBox2.Clear();
            client_item_list.Clear();
            // 11200	"11200"	"ITEM_ARMOR"	"ARMOR_BODY"	2	"ANTI_ASSASSIN | ANTI_SURA | ANTI_MUDANG"	"ITEM_TUNABLE"	"WEAR_BODY"	"NONE"	3000	3000	11201	163	15	"LEVEL"	0	"LIMIT_NONE"	0	"APPLY_MOV_SPEED"	0	"APPLY_NONE"	0	"APPLY_NONE"	0	0	12	0	3	0	0	0	1	0
            string querry = "";
            int id = Convert.ToInt32(textbox_id.Text);
            string type = box_type.GetItemText(box_type.SelectedItem);
            string type2 = box_subtype.GetItemText(box_subtype.SelectedItem);
            int size = Convert.ToInt32(box_size.Text);
            string antiflag = "";
            if (anti_00.Checked == true)
            {
                antiflag += " | ANTI_MUDANG";
            }
            if (anti_01.Checked == true)
            {
                antiflag += " | ANTI_ASSASSIN";
            }
            if (anti_02.Checked == true)
            {
                antiflag += " | ANTI_SURA";
            }
            if (anti_03.Checked == true)
            {
                antiflag += " | ANTI_MUSA";
            }
            if (anti_04.Checked == true)
            {
                antiflag += " | ANTI_SELL";
            }
            if (anti_05.Checked == true)
            {
                antiflag += " | ANTI_DROP";
            }
            if (anti_06.Checked == true)
            {
                antiflag += " | ANTI_GIVE";
            }
            if (anti_07.Checked == true)
            {
                antiflag += " | ANTI_STACK";
            }
            if (anti_08.Checked == true)
            {
                antiflag += " | ANTI_MYSHOP";
            }
            if (anti_10.Checked == true)
            {
                antiflag += " | ANTI_SAFEBOX";
            }
            if (anti_none.Checked == true)
            {
                antiflag += " | NONE";
            }
            if (anti_female.Checked == true)
            {
                antiflag += " | ANTI_FEMALE";
            }
            if (anti_male.Checked == true)
            {
                antiflag += " | ANTI_MALE";
            }
            if (String.IsNullOrEmpty(antiflag))
            {
                antiflag = "...NONE";
            }
            string item_type = "";
            if (flag_none.Checked == true)
            {
                item_type += " | NONE";
            }
            if (flag_tunable.Checked == true)
            {
                item_type += " | ITEM_TUNABLE";
            }
            if (flag_stackable.Checked == true)
            {
                item_type += " | ITEM_STACKABLE";
            }
            if (flag_slow_query.Checked == true)
            {
                item_type += " | ITEM_SLOW_QUERY";
            }
            if (flag_quest_give.Checked == true)
            {
                item_type += " | QUEST_GIVE";
            }
            if (flag_log.Checked == true)
            {
                item_type += " | LOG";
            }
            if (flag_unique.Checked == true)
            {
                item_type += " | ITEM_UNIQUE";
            }
            if (flag_count.Checked == true)
            {
                item_type += " | ITEM_MAKECOUNT";
            }
            if (flag_irremovable.Checked == true)
            {
                item_type += " | ITEM_IRREMOVABLE";
            }
            if (flag_confirm.Checked == true)
            {
                item_type += " | CONFIRM_WHEN_USE";
            }
            if (flag_quest_use.Checked == true)
            {
                item_type += " | QUEST_USE";
            }
            if (flag_quest_mult.Checked == true)
            {
                item_type += " | QUEST_USE_MULTIPLE";
            }
            if (flag_quest_item.Checked == true)
            {
                item_type += " | ITEM_QUEST";
            }
            if (flag_item_save.Checked == true)
            {
                item_type += " | ITEM_SAVE";
            }
            ...
            string wear_type = box_wearflag.GetItemText(box_wearflag.SelectedItem);
            string flag = "NONE";
            int price_sell = Convert.ToInt32(gold_sell.Text);
            int price_buy = Convert.ToInt32(gold_buy.Text);

            bool enable_refine_id = true;
            bool skaluj = false;
            
            int refine_id = id + 1;
            int refine_ingredients = Convert.ToInt32(textbox_refid.Text);
            int AlterToMagicItemPercent = Convert.ToInt32(text_AlterToMagicItemPercent.Text);
            string LimitType0 = box_limit1.GetItemText(box_limit1.SelectedItem);
            int LimitValue0 = Convert.ToInt32(box_limitval0.Text);
            string LimitType1 = box_limit2.GetItemText(box_limit2.SelectedItem);
            int LimitValue1 = Convert.ToInt32(box_limitval1.Text);
            string ApplyType0 = box_addon_00.GetItemText(box_addon_00.SelectedItem);
            float ApplyValue0 = Convert.ToInt32(addon_value_00.Text);
            float first_ApplyValue0 = ApplyValue0;
            string ApplyType1 = box_addon_01.GetItemText(box_addon_01.SelectedItem);
            float ApplyValue1 = Convert.ToInt32(addon_value_01.Text);
            float first_ApplyValue1 = ApplyValue1;

            string ApplyType2 = box_addon_02.GetItemText(box_addon_02.SelectedItem);
            float ApplyValue2 = Convert.ToInt32(addon_value_02.Text);
            float first_ApplyValue2 = ApplyValue2;

            int Value0 = Convert.ToInt32(value_0.Text);
            int Value1 = Convert.ToInt32(value_1.Text);
            int Value2 = Convert.ToInt32(value_2.Text);
            int Value3 = Convert.ToInt32(value_3.Text);
            int Value4 = Convert.ToInt32(value_4.Text);
            int Value5 = Convert.ToInt32(value_5.Text);
            int Value5_basic = Convert.ToInt32(value_5.Text);
            int specular = Convert.ToInt32(box_specular.Text);
            int GainSocketPercent = Convert.ToInt32(box_socket.Text);
            int AddonType = check_Addon();
            float Procent_value = (float)Convert.ToInt32(skaluj_percent_value.Text);
            string type_cs = "";
            if (box_type.GetItemText(box_type.SelectedItem) == "ITEM_ARMOR")
            {
                type_cs = "ARMOR";
            }
            else if (box_type.GetItemText(box_type.SelectedItem) == "ITEM_WEAPON")
            {
                type_cs = "WEAPON";
            }
            else
            ...
            for (int i = 0; i <= max; i++)
            {
                if (skaluj_main.Checked == true)
                {
                    skaluj = true;
                }
                if (skaluj && i > 0)
                {
                    if (skaluj_01.Checked == true)
                    {
                        ApplyValue0 += (first_ApplyValue0 * Procent_value)/100;
                        if (ApplyValue0 > Convert.ToInt32(max_01.Text))
                        {
                            ApplyValue0 = Convert.ToInt32(max_01.Text);
                        }
                    }
                    if (skaluj_02.Checked == true)
                    {
                        ApplyValue1 += (first_ApplyValue1 * Procent_value) / 100;
                        if (ApplyValue1 > Convert.ToInt32(max_02.Text))
                        {
                            ApplyValue1 = Convert.ToInt32(max_02.Text);
                        }
                    }
                    if (skaluj_03.Checked == true)
                    {
                        ApplyValue2 += (first_ApplyValue2 * Procent_value) / 100;
                        if (ApplyValue2 > Convert.ToInt32(max_03.Text))
                        {
                            ApplyValue2 = Convert.ToInt32(max_03.Text);
                        }
                    }
                }
                if (!(String.IsNullOrEmpty(textbox_refid.Text)))
                {
                    refine_id = id + 1;
                    refine_ingredients += 1;
                    Value5 += Value5_basic;
                }
                else
                {
                    refine_id = 0;
                    refine_ingredients = 0;
                }
                if (id % 10 == 9 || !enable_refine_id)
                {
                    refine_id = 0;
                    refine_ingredients = 0;
                }
                querry = id + "	\"" + id + "\"" + "	\"" + type + "\"" + "	\"" + type2 + "\"" + "	" + size + "" + "	\"" + antiflag.Substring(3) + "\"" + "	\"" + item_type.Substring(3) + "\"" + "	\"" + wear_type + "\"" + "	\"" + flag + "\"" + "	" + price_sell + "" + "	" + price_buy + "" + "	" + refine_id + "" + "	" + refine_ingredients + "" + "	" + AlterToMagicItemPercent + "" + "	\"" + LimitType0 + "\"" + "	" + LimitValue0 + "" + "	\"" + LimitType1 + "\"" + "	" + LimitValue1 + "" + "	\"" + ApplyType0 + "\"" + "	" + Convert.ToInt32(ApplyValue0) + "" + "	\"" + ApplyType1 + "\"" + "	" + Convert.ToInt32(ApplyValue1) + "" + "	\"" + ApplyType2 + "\"" + "	" + Convert.ToInt32(ApplyValue2) + "" + "	" + Value0 + "" + "	" + Value1 + "" + "	" + Value2 + "" + "	" + Value3 + "" + "	" + Value4 + "" + "	" + Value5 + "" + "	" + specular + "" + "	" + GainSocketPercent + "" + "	" + AddonType + "";
                richTextBox1.AppendText(querry + Environment.NewLine);
                CreateList(id, textbox_nazwa.Text);
                CreateList_client(id, type_cs, "sanguine/icon/" + icon_patch.Text, text_groundpatch.Text);
                
                id += 1;


            }
            if (!String.IsNullOrEmpty(icon_patch.Text))
            {
                if (File.Exists(@"C:\Sanguine_file_copy\sanguine\icon\" + openFileDialog1.SafeFileName))
                    File.Delete(@"C:\Sanguine_file_copy\sanguine\icon\" + openFileDialog1.SafeFileName);
                File.Copy(openFileDialog1.FileName, @"C:\Sanguine_file_copy\sanguine\icon\" + openFileDialog1.SafeFileName);
            }
            if (!String.IsNullOrEmpty(text_groundpatch.Text))
            {
                if (File.Exists(@"C:\Sanguine_file_copy\meshes\item\" + openFileDialog2.SafeFileName))
                    File.Delete(@"C:\Sanguine_file_copy\meshes\item\" + openFileDialog2.SafeFileName);
                File.Copy(openFileDialog2.FileName, @"C:\Sanguine_file_copy\meshes\item\" + openFileDialog2.SafeFileName);
            }
        }
        ...
        private void savefiletopatch(object sender, CancelEventArgs e)
        {
            icon_patch.Text = openFileDialog1.SafeFileName;
            this.pictureBox1.Image = Paloma.TargaImage.LoadTargaImage(@openFileDialog1.FileName);
        ...
        {
            text_groundpatch.Text = openFileDialog2.SafeFileName;
        }
